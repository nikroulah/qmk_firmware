...
custom Miryoku keymaps for three boards: the **bastardkb skeletyl (v1/elitec)**
(`split_3x5_3`, 6 thumbs), the **ferris/sweep** (`split_3x5_2`, 4 thumbs), and
the **ZSA Voyager** (52-key `LAYOUT`, 2 thumbs/side — reuses the sweep layout).
All are Miryoku builds selected per board, built with **`-km nikroulah`**.
...
**`users/nikroulah/`**, **`layouts/community/split_3x5_{3,2}/nikroulah/`** (the
skeletyl/sweep entry points), and **`keyboards/zsa/voyager/keymaps/nikroulah/`**
(the Voyager entry point — it isn't a community-layout board).
...
- **voyager**: `zsa/voyager`, STM32F303 (ARM, ~256 KB flash — lots of room),
  unibody-wired split via mcp23018 (single controller, no `EE_HANDS`). Physical
  `LAYOUT` is 52 keys (6 cols × 4 rows + 2 thumbs/side). Only 2 thumbs/side, so
  it **reuses the sweep (`split_3x5_2`) layer macros** — selected in
  `users/nikroulah/config.h` by `defined(KEYBOARD_zsa_voyager)`. It is **not** a
  community-layout board, so its entry point is a real keymap dir:
  `keyboards/zsa/voyager/keymaps/nikroulah/` (empty `keymap.c`, a comment-only
  `rules.mk`, and a `config.h` that defines `LAYOUT_miryoku` → `LAYOUT`). The
  mapping uses Voyager rows 1–3 (top row 0, the number row, is unused), the
  **inner 5 finger columns each hand** (the outer extra column x=0/x=15 — which
  stock Voyager gives to Backspace/mods — is unused), and both thumbs/side
  (`K33 K34`→left outer/inner, `K35 K36`→right inner/outer, matching the sweep).
  Unused physical keys are `KC_NO`; the right pinky `'` lands on the physical `;`
  cap (normal 5-on-6 tradeoff). Firmware ~57 KB.
- The skeletyl/sweep declare the matching `community_layouts`, so their keymap
  entry point comes from `layouts/community/split_3x5_{3,2}/nikroulah/` (an empty
  `keymap.c` plus the `LAYOUT_miryoku` mapping in `config.h`; the actual keymap is
  generated by the babel engine). There is **no** `keymaps/nikroulah/` dir under
  either of those boards (unlike the Voyager, which needs one).
...
# build (voyager)
qmk compile -kb zsa/voyager -km nikroulah
...
- **Tab-switching keys on NAV**, arranged around the left-hand Alt/Cmd mods so
  ...
  under Alt, `Ctrl+Tab` under Cmd (`LCTL(LSFT(KC_TAB))` / `LCTL(KC_TAB)`). (The
  top-row `Cmd+Shift+Tab` / `Cmd+Tab` that briefly sat above these mods were
  removed; those two slots are back to `U_NA`. MOUSE keeps its own `Ctrl+Tab` /
  `Ctrl+Shift+Tab` on the bottom row.)
...
## ZSA community modules + Navigator trackpad (Voyager only)

We forward-ported ZSA's `qmk_modules` and the Navigator **trackpad (PTP
multitouch)** onto our 0.33.7 base (ZSA's `firmware25` fork is ~1064 commits
*behind* us, so adopting their base would be a downgrade — we port onto ours).

- **Vendored modules** live in **`modules/zsa/`** (a copy of `zsa/qmk_modules`,
  pinned to the commit `firmware25` uses: `d618614`). QMK's community-module
  build system (present in 0.33.7) picks them up.
- **How modules attach to our C keymap:** the Voyager keymap dir has a
  **modules-only `keymap.json`** (`{"modules": [...]}`, no `layers`) sitting
  **beside** the introspection `keymap.c`. `build_keyboard.mk` supports that
  coexistence (`OTHER_KEYMAP_C`), so modules load without converting our
  Miryoku keymap to JSON. Skeletyl/sweep load **no** modules.
- **Enabled on the Voyager:** `zsa/defaults` (custom keycodes), `zsa/navigator_trackpad`
  (the Cirque Gen6 PTP driver), `zsa/doubleclick`, `zsa/mousejiggler`. The
  trackpad works (cursor + gestures) with no keymap keys; its `TP_CPIU`/`MJ_TOGG`/
  `KC_MS_DBL_CLICK` keycodes are compiled in but **unplaced**. `automouse` and
  `navigator_trackball` are vendored but not loaded (trackpad is a *digitizer*,
  not a pointing device; trackball is the other hardware). `keycolors` needs a
  user `ledmap[][]`; `oryx` (Keymapp live view) conflicts with our qmk_viewer raw
  block — both left off.
- **Forward-port edits to vendored modules** (API drift vs firmware25's older
  base): `navigator_trackball.c` `pointing_device_driver_init`→`bool`;
  `doubleclick.c` `KC_MS_BTN1`→`MS_BTN1`; `defaults/defaults.h` dropped its
  `TOGGLE_LAYER_COLOR`/`LED_LEVEL` enum (mainline `voyager.h` owns them).
- **PTP core forward-port** (multitouch needs core, not just the module): ~10
  core files carry gated additions, **all behind `#ifdef DIGITIZER_MODE_TOUCHPAD`**
  (set by the module's `DIGITIZER_MODE = touchpad` → `protocol.mk`): `report.h`,
  `quantum/digitizer.{c,h}`, `quantum/keyboard.c`, `tmk_core/protocol.mk`, the
  **new** `tmk_core/protocol/digitizer.h`, `host.c`, `usb_descriptor.{c,h}` (the
  PTP HID descriptor `DigitizerTouchpadReport[]`), and chibios `usb_main.c` +
  `usb_report_handling.c`. Grep `nikroulah forward-port` to find every hunk.
- **Gating guarantee:** because every core hunk is `DIGITIZER_MODE_TOUCHPAD`-gated
  and no board but the Voyager-with-trackpad defines it, the AVR skeletyl/sweep
  `.hex` is **byte-identical** before/after the port. Re-verify after any core
  edit: `shasum` the skeletyl/sweep `.hex` against a pre-change build.
- **Status:** builds clean (Voyager ~68 KB); **not yet flash-tested** — PTP
  descriptor/enumeration correctness is only verifiable on hardware.
- Re-sync the vendored core hunks if you ever bump QMK (like the other engine
