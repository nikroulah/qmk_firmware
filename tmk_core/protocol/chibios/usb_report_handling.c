...
// nikroulah forward-port (zsa firmware25): PTP feature-report handling.
#    include "digitizer.h"
#endif
...
#ifdef DIGITIZER_MODE_TOUCHPAD
    // nikroulah forward-port (zsa firmware25): touchpad feature reports use the
    // digitizer interface's own report-ID space (1-5), so handle them before the
    // generic IS_VALID_REPORT_ID check that would reject them.
