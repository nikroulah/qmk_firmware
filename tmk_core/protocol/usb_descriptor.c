...
// nikroulah forward-port (zsa firmware25): the standard single-point digitizer
// descriptor is excluded in touchpad (PTP) mode; the multitouch descriptor below
// replaces it.
...
// nikroulah forward-port (zsa firmware25): Precision Touchpad multitouch HID
// descriptor. The per-finger collection is a macro; the report array itself is
// declared after SharedReport closes (below) so it never nests inside it.
#    include "digitizer.h"
...
#define PTP_FINGER_COLLECTION                                                              \
    ...
    HID_RI_COLLECTION(8, 0x00),        /* Physical */                                      \
        ...
        HID_RI_POP(0),                                                                     \
    HID_RI_END_COLLECTION(0)
#endif // DIGITIZER_MODE_TOUCHPAD (PTP_FINGER_COLLECTION macro)

#ifdef DIGITIZER_MODE_TOUCHPAD
// Digitizer/Touchpad HID Descriptor. Standalone array (dedicated endpoint), so
// it does not nest inside SharedReport.
...
#endif // DIGITIZER_MODE_TOUCHPAD (DigitizerTouchpadReport)
